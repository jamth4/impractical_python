...
def main():
    '''Load files, run filters, all user to view anagrams by 1st letter'''
    name = 'tmvoordle'
    name = name.lower()

    word_list_ini = load_dictionary.load('solving_anagrams//words.txt')
    trigrams_filtered = load_dictionary.load('solving_anagrams//least-likely_trigrams.txt')
...
def cv_map_words(word_list):
    ...
    n = int(total * target)
    count_prunted = Counter(cv_mapped_words).most_common(total - n)
    filtered_cv_map = set()
    for pattern, count in count_prunted:
